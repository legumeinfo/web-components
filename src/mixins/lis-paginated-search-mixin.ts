 ...
 */
export type LisPaginatedSearchResults<SearchResult> = {
  pageSize?: number;
  hasNext?: boolean;
  numResults?: number;
  numPages?: number;
 ...
 */
export type LisPaginatedSearchData = {page: number};
 ...
 */
export type LisPaginatedSearchOptions = {abortSignal?: AbortSignal};
 ...
 * array of the {@link LisPaginatedSearchResults | `LisPaginatedSearchResults`}
 ...
 * but may be useful.
 */
export type LisPaginatedSearchFunction<SearchData, SearchResult> = (
  searchData: SearchData & LisPaginatedSearchData,
  options: LisPaginatedSearchOptions,
) => Promise<LisPaginatedSearchResults<SearchResult>>;
 ...
 */
export type LisPaginatedDownloadResults = {
  errors?: string[];
};
 ...
 * but may be useful.
 */
export type LisPaginatedDownloadFunction<SearchData> = (
  searchData: SearchData,
  options: LisPaginatedSearchOptions,
) => Promise<LisPaginatedDownloadResults>;
...
> {
   ...
   */
  searchFunction: LisPaginatedSearchFunction<SearchData, SearchResult>;
   ...
   */
  downloadFunction?: LisPaginatedDownloadFunction<SearchData>;
   ...
   * attribute of the {@link LisPaginatedSearchOptions | `LisPaginatedSearchOptions`}
   * object passed to the component's {@link searchFunction | `searchFunction`}
   * and {@link LisPaginatedDownloadFunction | `LisPaginatedDownloadFunction`}..
   */
...
export const LisPaginatedSearchMixin =
  ...
  <SearchData, SearchResult>() => {
    ...
    class LisPaginatedSearchElement extends superClass {
      ...
      @state()
      private _searchData: (SearchData & LisPaginatedSearchData) | undefined =
        undefined;
      ...
      private _search(
        searchData: (SearchData & LisPaginatedSearchData) | undefined,
      ): void {
        ...
        this.cancelPromiseController.wrapPromise(searchPromise).then(
          (results: LisPaginatedSearchResults<SearchResult>) =>
      ...
      private _searchSuccess(
        paginatedResults: LisPaginatedSearchResults<SearchResult>,
      ): void {
      ...
      protected _download(formData: SearchData): void {
        if (this.downloadFunction !== undefined) {
          ...
          this.cancelPromiseController.wrapPromise(downloadPromise).then(
            (results: LisPaginatedDownloadResults) =>
              this._downloadSuccess(results),
            (error: Error | Event) => {
              // do nothing if the request was aborted
              if (!(error instanceof Event && error.type === 'abort')) {
      ...
      // updates the table and loading element with the search result data
      protected _downloadSuccess(
        downloadResults: LisPaginatedDownloadResults,
      ): void {
